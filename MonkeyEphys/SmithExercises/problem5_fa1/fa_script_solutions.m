...
%% crossvalidate over zdims
   params =  cv_fastfa_solution(counts(neuroninds,trialinds),zdims,4);

%% compute d_shared and percent shared
    [percentsharedt(tind), d_sharedt(tind) bestdimt(tind),sharedvar] = compute_shared_solution(params, thresh);
...
figure
scatter(1:length(maxsharedvar),cumsum(maxsharedvar/sum(maxsharedvar)))
