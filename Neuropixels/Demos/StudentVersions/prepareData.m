...
% hint relevant variables: S.spikes.times, s.trials.intervals

% pre-allocate
neurons.rate = zeros(trials.N,neurons.N);
% use for loops to fill in values of this variable:
...
%% Exercise: scatter plot mean firing rate versus depth
...
figure
for p = 0:1 % separate subplots for each probe
    subplot(2,1,p+1)
    hold on
    for r = 1:regions.N 
       % plot all spikes in a given region against depth
       % add a plot command on the next line
...
%% Exercise: Plot the smoothing kernel versus tmesh
...
% use bar plot for binned
% use plot for smoothed
% make sure 
